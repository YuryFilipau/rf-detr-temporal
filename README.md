...
Модификация RF-DETR для детекции автомобилей на последовательностях кадров. Обычный RF-DETR работает как image detector: один входной кадр, один набор признаков, один набор query, один прогноз bbox/class. В этой работе добавлен temporal-контекст: вместе с текущим кадром модель может использовать соседние кадры из того же видео.
