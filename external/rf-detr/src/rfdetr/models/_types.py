...
'''
здесь обновлен список аргументов, которые ожидает сборщик модели.
без этих полей temporal параметры могли бы потеряться между конфигом,
namespace и build_model, поэтому тип явно знает о temporal настройках.
'''
...
class BuilderArgs(Protocol):
    ...
    num_queries: int
    num_select: int
    temporal_num_ref_frames: int  # нужно build_model, чтобы понять включать ли temporal ветку
    temporal_fusion_layers: int  # нужно temporal module для числа attention слоев
    temporal_dropout: float  # нужно temporal module для dropout внутри fusion
