...
'''
этот файл связывает новые параметры temporal с внутренним namespace rf-detr.
rf-detr строит модель через старый набор аргументов, поэтому сюда добавлены
temporal_num_ref_frames, temporal_fusion_layers и temporal_dropout, чтобы они
доходили из конфига до модели без ручных правок в каждом месте.
'''
...
_MC_NAMESPACE_FIELDS = {
    ...
    "sa_nheads",
    "segmentation_head",
    "temporal_dropout",  # передает dropout temporal attention в сборщик модели
    "temporal_fusion_layers",  # передает число слоев temporal fusion
    "temporal_num_ref_frames",  # передает количество соседних кадров
    "two_stage",
}
