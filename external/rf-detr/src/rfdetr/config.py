...
'''
здесь расширен конфиг rf-detr: добавлены параметры для temporal режима.
они позволяют включать соседние кадры, задавать их количество, способ выбора
кадров и число слоев temporal fusion без изменения основного кода обучения.
'''
...
class ModelConfig(BaseConfig):
    ...
    temporal_num_ref_frames: int = Field(  # сколько соседних кадров добавлять к текущему кадру
        default=0,  # 0 оставляет обычный rf-detr без temporal ветки
        ge=0,  # отрицательное число кадров не имеет смысла
        description=(
            "Number of reference frames for temporal RF-DETR. "
            "0 keeps standard image RF-DETR. N expects inputs packed as "
            "3*(N+1) RGB channels: key frame first, then reference frames."
        ),
    )
    temporal_fusion_layers: int = Field(  # сколько раз применять attention между текущим и соседними кадрами
        default=1,  # один слой выбран как легкий вариант для gpu на 8 gb
        ge=1,  # хотя бы один слой нужен, если temporal включен
        description="Number of TransVOD-style temporal query fusion layers.",
    )
    temporal_dropout: float = Field(  # dropout внутри temporal attention
        default=0.0,  # по умолчанию не добавляем лишнюю регуляризацию
        ge=0.0,  # нижняя граница вероятности dropout
        le=1.0,  # верхняя граница вероятности dropout
        description="Dropout used inside temporal query fusion.",
    )
...
class TrainConfig(BaseModel):
    ...
    temporal_ref_frame_mode: Literal["previous", "surrounding", "duplicate"] = Field(  # способ выбора соседних кадров
        default="previous",  # безопасный режим для автопилота: модель не видит будущие кадры
        description=(
            "Reference-frame sampling for temporal RF-DETR datasets. "
            "'previous' is causal and recommended for autonomous driving; "
            "'surrounding' may use future frames for offline validation; "
            "'duplicate' repeats the key frame as a no-motion control."
        ),
    )
    temporal_ref_frame_step: int = Field(  # расстояние между текущим кадром и выбранными соседними
        default=1,  # берем ближайшие кадры, если явно не задан другой шаг
        ge=1,  # шаг 0 означал бы тот же самый кадр
        description="Frame index step between the key frame and sampled references.",
    )
